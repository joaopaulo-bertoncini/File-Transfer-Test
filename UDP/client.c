...
int main (int argc, char**argv){
    ...
    int sfd,fd;
    char buf[BUFFERT],buf_recieved[10];
    off_t count=0, sock_data,sz;
	long int fp;
    int l=sizeof(struct sockaddr_in);
	struct stat buffer;
    int n_send = 0;
    int sleep_time = 100; // tempo de espera em milisegundos 
	...
	while(fp){
    	...
    	usleep(sleep_time); // aguarda milisecond
	    n_send++;	
		count+=sock_data;
		bzero(buf,BUFFERT);
        fp=read(fd,buf,BUFFERT);
